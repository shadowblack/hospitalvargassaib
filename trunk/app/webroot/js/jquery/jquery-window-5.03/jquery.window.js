...
jQuery.Window = (function()  {
	...
	function constructor(caller, options) {	       
		...
		function initialize(instance) {	                   
			...
			if( jQuery.browser.msie ) { // To fix the right or bottom edge of window can't be trigger to resize while scrollbar appears on IE browser
				container.css({
					paddingRight: 1,
					paddingBottom: 1
				});
		...
		function maximize(bImmediately, bNoSaveDisplay) {
			if( !jQuery.browser.msie && caller == null ) { // in IE, must do hide scrollbar routine after animation finished
