...
/**
 *  Enumera y recuerda en la clase el contador de las secuencias para las paginaciones
 * @var integer
 */ 
    var $NumRowPre = 0;
...
/**
 * Obtiene los números de registros siguiendo su secuencia a lo largo de las paginas
 * @return integer
 * @access public 
 * @param $1 contador: por defecto es 1, $2 start: a partir de donde comienza a contar
 * 
 */
    function NumRowPre($cont = 1, $start = 1){  
        $start =  $start-$cont;
        $sum =  $this->NumRowPre;
        $sum = $sum + $cont;
        $this->NumRowPre = $sum;         
        $paging = $this->params($this->defaultModel());   
        return $row_start = (($paging['options']['page']-1) * $paging['options']['limit'] * $cont + $sum + $start);                 
    }
